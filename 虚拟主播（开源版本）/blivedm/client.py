...
# Add default image fingerprint parameters to bypass -352 wind-control (see GitHub issue #868).
FINGERPRINT_PARAMS = {
    'dm_img_list': '[]',
    'dm_img_str': 'V2ViR0wgMS',  # "WebGL 1" base64-encoded
...
class BLiveClient:
    ...
    def __init__(
        ...
        self._csrf_token = None  # 从cookie bili_jct获取
    ...
    async def init_room(self):
        ...
        :return: True 始终返回 True，只要基本字段已填充；将是否降级信息通过日志提示。
        """
        downgraded = False
        ...
        if not await self._init_host_server():
            ...
            self._host_server_token = None
        if downgraded:
            logger.warning('room=%s init_room downgraded, may affect reliability', self._tmp_room_id)
        return True

    async def _init_room_id_and_owner(self):
        try:
            url1 = ROOM_INIT_URL
            async with self._session.get(url1, params={**{'room_id': self._tmp_room_id}, **FINGERPRINT_PARAMS}, ssl=self._ssl) as res:
                if res.status != 200:
            ...
            try:
                async with self._session.get('https://api.live.bilibili.com/room/v1/Room/room_init', params={**{'id': self._tmp_room_id}, **FINGERPRINT_PARAMS}, ssl=self._ssl) as res:
                    if res.status != 200:
                        return False
                    data = await res.json()
                    if data['code'] != 0:
                ...
                logger.exception('room=%d _init_room_id_and_owner() fallback failed:', self._tmp_room_id)
                    return False
    ...
    def _parse_room_init(self, data):
        """解析房间初始化信息，兼容旧 /room_init 与新 /getInfoByRoom 格式。"""
        if 'room_info' in data:  # 新接口
        room_info = data['room_info']
        else:  # 旧接口直接返回 room_id 等字段
            room_info = data

        try:
            self._room_id = room_info.get('room_id') or room_info.get('roomid') or room_info.get('rid')
            self._room_short_id = room_info.get('short_id', 0)
            self._room_owner_uid = room_info.get('uid') or room_info.get('owner_uid') or 0
            if not self._room_id:
                raise ValueError('room_id missing')
        return True
        except Exception:
            logger.exception('room=%s _parse_room_init() failed: %s', self._tmp_room_id, room_info)
            return False

    async def _init_host_server(self):
        try:
            url1 = DANMAKU_SERVER_CONF_URL
            async with self._session.get(url1, params={**{'id': self._room_id, 'type': 0}, **FINGERPRINT_PARAMS}, ssl=self._ssl) as res:
                if res.status != 200:
            ...
            # fallback to getConf
            try:
                async with self._session.get('https://api.live.bilibili.com/room/v1/Danmu/getConf', params={**{'room_id': self._room_id, 'platform': 'pc', 'player': 'web'}, **FINGERPRINT_PARAMS}, ssl=self._ssl) as res:
                    if res.status != 200:
                        return False
                    data = await res.json()
                    if data['code'] != 0:
                ...
                logger.exception('room=%d _init_host_server() fallback failed:', self._room_id)
                    return False
        except (aiohttp.ClientConnectionError, asyncio.TimeoutError):
            ...
            if header.ver == ProtoVer.BROTLI:
                ...
                if len(body) != 0:
                    try:
                        ...
                        raise
            elif header.ver == ProtoVer.ZSTD:
                # 使用zstandard解压
                dctx = zstandard.ZstdDecompressor()
                decompressed_body = dctx.decompress(body)
                await self._parse_ws_message(decompressed_body)
            else:
                # 未知格式
    ...
    def _get_csrf_from_cookies(self) -> Optional[str]:
        """尝试从当前 session 的 CookieJar 中获取 bili_jct 作为 csrf token"""
        try:
    ...
    async def send_danmaku(self, message: str):
        """向当前房间发送弹幕，需要登录后的 SESSDATA 与 bili_jct Cookie。

        :param message: 弹幕文本，最长100字符左右
        :raise RuntimeError: 发送失败时抛出
        """
        if self._room_id is None:
            raise RuntimeError('send_danmaku() should be called after init_room()')

        csrf = self._get_csrf_from_cookies()
        if not csrf:
            logger.error('room=%d send_danmaku() failed: no bili_jct cookie found', self._room_id)
            raise RuntimeError('csrf token (bili_jct) not found in session cookies')
        ...
        try:
            async with self._session.post('https://api.live.bilibili.com/msg/send', data=payload, ssl=self._ssl) as res:
                if res.status != 200:
                    logger.warning('room=%d send_danmaku() http error %d %s', self._room_id, res.status, res.reason)
                    raise RuntimeError(f'http {res.status} {res.reason}')
                result = await res.json()
                if result.get('code', 0) != 0:
                    logger.warning('room=%d send_danmaku() api error %s', self._room_id, result)
                    raise RuntimeError(result.get('message', 'unknown error'))
                return result
        except (aiohttp.ClientConnectionError, asyncio.TimeoutError):
            logger.exception('room=%d send_danmaku() network error:', self._room_id)
            raise
